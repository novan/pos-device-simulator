...
public class SimulatedCashDrawerService extends AbstractSimulatedService implements CashDrawerService111 {
    ...
    public void toggleDrawer(boolean open) {
        if (open) {
            ...
            SimulatedCashDrawerPanel.getInstance().getBtnClose().setEnabled(true);
            SimulatedCashDrawerPanel.getInstance().updateStatusLabel(this.cashDrawerOpened);
        }
        
        else {
            this.cashDrawerOpened = false;
            SimulatedCashDrawerPanel.getInstance().getBtnClose().setEnabled(false);
            SimulatedCashDrawerPanel.getInstance().updateStatusLabel(this.cashDrawerOpened);
        }
    }
    ...
    public void openDrawer() throws JposException {
        if (this.cashDrawerOpened) throw new JposException(JposConst.JPOS_E_FAILURE, "Cash Drawer Already Opened");
        toggleDrawer(true);
