...
public class DecimalFormatter {
    ...
    public static String formatCurrency(BigDecimal currencyValue, Locale locale, boolean includeCurrencySymbol) {
        return formatCurrency(currencyValue, locale, includeCurrencySymbol, true);
    }
    ...
    public static String formatCurrencyAsTicketReprint(BigDecimal currencyValue, Locale locale) {
        ...
        if (locale.getISO3Country().equals("USA") || locale.getISO3Country().equals("CAN")) {
        ...
        if (currencyValue.toString().indexOf(symbols.getMonetaryDecimalSeparator()) != -1) {
            double mantissa = Double.parseDouble(currencyValue.toString().substring(
                    currencyValue.toString().indexOf(symbols.getMonetaryDecimalSeparator())));
            if (mantissa == 0.0d) {
                int decimalLocation = formattedValue.lastIndexOf(symbols.getMonetaryDecimalSeparator());
